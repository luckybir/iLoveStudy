...
var newtiku = "newtiku.db";  //自定义备份为新题库名，建议配合刷题js用
...
/**
 * @description: 恢复/备份题库文件
 * @param: bakpath, newtiku,restoreOrBackup0:restore,1:backup
 * @return: true/false
 */
function backupTiku(restoreOrBackup, bakpath, newtiku) {//备份题库文件
    let oldtikupath = files.path("tiku.db");
    let newtikupath = bakpath + newtiku;
    if (restoreOrBackup == 0) {//restore
        if (files.exists(newtikupath)) {
            files.copy(newtikupath,oldtikupath);
            console.log("已导入题库" + newtiku + "成功");
            return true;
        }
    }
    if (restoreOrBackup == 1) {//backup
        if (files.exists(oldtikupath)) {
            let f = 0;
            while (files.exists("./tiku.db-shm") && files.exists("./tiku.db-wal")) {
                //数据库未关闭
                if (f == 1) {//1秒时关闭数据库
                    let db = SQLiteDatabase.openOrCreateDatabase(oldtikupath, null);
                    db.close();
                }
                if (f % 5 == 0) {
                    console.log("已等待" + (f + 1) + "秒，正在等待数据库正常关闭...");
                }

                if (f == 30) {
                    console.log("已超时" + (f + 1) + "秒，将直接复制备份题库文件...");
                    break;
                }
                delay(1);
                f++;
            }
            files.copy(oldtikupath, newtikupath);
            console.log("备份题库为" + newtiku + "成功");
            return true;
        }
    }
    return false;
...
function main() {
    ...
    //导入题库
    if (xxset.xxbackup) {
        if (backupTiku(0, bakpath, newtiku)) {//复制自己的题库过来
            DLog = detailLog(DLog, "导入题库" + newtiku);
        }
    }
    ...
    //备份题库
    if (xxset.xxbackup) {
        if (backupTiku(1, bakpath, newtiku)) {//复制自己的题库过来
            DLog = detailLog(DLog, "备份题库为" + newtiku);
        }
    }
