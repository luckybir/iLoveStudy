...
function getLearnedArticle(title, date) {
  ...
  var sql =
    "SELECT * FROM  learnedArticles WHERE title = '" +
    rtitle +
    "' AND date = '" +
    date +
    "'";
...
function insertLearnedArticle(title, date) {
  ...
  db.execSQL(createTable);
  var sql =
    "INSERT INTO learnedArticles VALUES ('" + rtitle + "','" + date + "')";
  db.execSQL(sql);
  db.close();
}
  ...
  for (var i = 0, t = 0; i < aCount; ) {
    ...
    if (click(s, t) == true) {
      ...
      if (id("xxqg-article-header").exists()) {
        ...
        currentNewsTitle = textContains("来源")
          .findOne()
          .parent()
          .children()[0]
          .text();
      } else if (textContains("作者").exists()) {
        currentNewsTitle = textContains("作者")
          .findOne()
          .parent()
          .children()[0]
          .text();
      } else if (descContains("来源").exists()) {
        currentNewsTitle = descContains("来源")
          .findOne()
          .parent()
          .children()[0]
          .desc();
      } else if (descContains("作者").exists()) {
        currentNewsTitle = descContains("作者")
          .findOne()
          .parent()
          .children()[0]
          .desc();
...
function videoStudy_bailing(vCount, vTime) {
  ...
  for (var i = 0; i < vCount; ) {
    ...
    while (true) {
      try {
        if (textContains("分享").exists) {
          //百灵视频
          currentNewsTitle = textContains("分享")
            .findOne()
            .parent()
            .parent()
            .parent()
            .children()[1]
            .text(); //10.29测试，央视网/中央广播电视总台 视频标题
    ...
    if (video_timing_bailing(i, vTime)) {
      //观看百灵小视频成功
      ////观看成功，记录已观看的视频
      insertLearnedArticle(currentNewsTitle, date_string);
...
function videoStudy_news() {
  ...
  for (var i = 0, t = 1; i < vCount; ) {
    ...
    if (click(s, t) == true) {
      ...
      if (textContains("展开").exists) {
        if (s == "央视网" || s == "中央广播电视总台") {
          currentNewsTitle = textContains("展开")
            .findOne()
            .parent()
            .parent()
            .parent()
    ...
    try {
      className("android.widget.ListView")
        ...
        .forEach((item) => {
          ...
          myScores[name] = score;
          xxScores = Number(
            textStartsWith("今日已累积").findOnce().text().substr(6, 2)
          ); //累计学习积分
        });
      err = true;
    } catch (e) {
...
function start_app(x) {
  ...
  while (!err) {
    //等待强国主页加载
    try {
      if (app.getAppName(currentPackage()) != "学习强国") {
        //如果强国未启动
        if (x == undefined) {
          //不带参数时输出信息
          console.log("启动学习强国");
...
function main() {
  ...
  if (customize_flag) {
    //自定义
    var funArr = [xx1, xx2, xx3, xx4, xx5, xx6, xx7, xx8, xx9, xx10],
      func; //随机学习所有模块，即含每周答题和专项答题
  } else {
    var funArr = [xx1, xx2, xx3, xx4, xx5, xx6, xx9, xx10],
      func; //随机学习各模块，不含每周答题和专项答题
  }
 ...
 * @return: null
 */
function UpdateOrDeleteTK(upOrdel, question, answer) {
  //只针对tiku表，添加/更新或删除相应的列
  ...
  if (upOrdel == "up" || upOrdel == 1) {
    ...
    let sql1 =
      "SELECT answer FROM tiku WHERE question LIKE '%" + question + "%'"; // 关键词前后都加%，增加搜索准确率
...
function SRQuestion() {
  ...
  while (true) {
    if (
      className("android.view.View").text("继续挑战").exists() ||
      textContains("继续挑战").exists() ||
      textContains("请明日再来").exists() ||
      className("android.view.View").text("非积分奖励局").exists()
    ) {
      //遇到继续挑战，则本局结束
